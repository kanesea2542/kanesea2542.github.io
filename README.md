...
`assets/ble-beacon/board-model.json` and `board-model.bin` contain geometry and colors converted from the supplied `PCB2.step` with occt-import-js 0.0.23 (used locally during conversion, not shipped). `board-overview.png` is an orthographic top rendering used as the 3D viewer poster. The homepage uses the supplied Altium screenshot, `altium-overview.png`, which includes copper and silkscreen artwork absent from the STEP export. The BLE gallery includes this screenshot, layouts with and without polygon pours, and the manufactured bare and populated board photos. All five PNGs are unmodified copies of the supplied screenshots.

The zoomable schematic is rendered from the original PDF. The PDF and source STEP file remain in `assets/ble-beacon/`, but the website provides an interactive model instead of a STEP download button.

The signature analyzer page is `projects/hp5004a.html`. Its homepage preview and zoomable system diagram use the original `assets/hp5004a/system-diagram.png`. Image viewers share `projects/ble-beacon.js`. The supplied `wave.asdb` is available as an original simulation-data download. The waveform viewer displays the supplied PNG export, `assets/hp5004a/waveform.png`, with zoom, drag-to-pan, scrollbars, and an original-image link. This is a static image, not an interactive simulation. Both PNG files are unchanged copies of the supplied images. The two remaining projects have text cards pending images and are labeled in progress. No performance metrics or completion dates are inferred.
