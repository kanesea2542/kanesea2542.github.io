...
The signature analyzer page is `projects/hp5004a.html`. Its homepage preview and zoomable system diagram use the original `assets/hp5004a/system-diagram.png`. Image viewers share `projects/ble-beacon.js`. The supplied `wave.asdb` is available as an original simulation-data download. A browser waveform image is still pending: ASDB needs compatible Aldec software to render or convert; a waveform screenshot/PDF or VCD export can be added when available. No waveform is synthesized from the block diagram. The two remaining projects have text cards pending images and are labeled in progress. No performance metrics or completion dates are inferred.
